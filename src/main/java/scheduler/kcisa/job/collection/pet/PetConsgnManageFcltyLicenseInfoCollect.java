...
public class PetConsgnManageFcltyLicenseInfoCollect extends QuartzJobBean {
    ...
    protected void executeInternal(@NotNull JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        JobUtils.executeJob(jobExecutionContext, tableName, jobData -> {
            JsonNode response = PetCollection.getPetData(url, "src/main/resources/data/pet/", stdDateStr + "_PetConsgnManageFcltyLicenseInfo.zip", stdDateStr + "_PetConsgnManageFcltyLicenseInfo.csv");

            String sql = Utils.getSQLString("src/main/resources/sql/collection/pet/PetConsgnManageFcltyLicenseInfo.sql");
            ...
            try (PreparedStatement pstmt = jobData.conn.prepareStatement(sql);) {
                ...
                System.out.println(LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd")) + " 동물위탁 관리 시설 수집 완료");
