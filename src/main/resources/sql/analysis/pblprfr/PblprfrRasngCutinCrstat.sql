INSERT INTO
	analysis_model.pblprfr_rasng_cutin_crstat
...
, SUM(IF(GENRE_CD = 'GGGA', RASNG_CUTIN_CO,
         0))                    AS MUSICL_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'CCCA', RASNG_CUTIN_CO,
         0))                    AS CLSIC_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'CCCC', RASNG_CUTIN_CO,
         0))                    AS KCLSIC_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'CCCD', RASNG_CUTIN_CO,
         0))                    AS POPULAR_MUSIC_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'BBBC', RASNG_CUTIN_CO,
         0))                    AS DANCE_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'BBBE', RASNG_CUTIN_CO,
         0))                    AS POPULAR_DANCE_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'EEEB', RASNG_CUTIN_CO,
         0))                    AS CIRCUS_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'EEEA', RASNG_CUTIN_CO,
...
, (CASE SUM(PBLPRFR_CO)
	   WHEN 0 THEN 0
	   ELSE IFNULL(SUM(RASNG_CUTIN_CO) /
	               SUM(PBLPRFR_CO) * 100,
	               0) END) AS RASNG_CUTIN_RATE
, SUM(IF(GENRE_CD = 'AAAA', RASNG_CUTIN_CO,
         0))             AS THTRE_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'GGGA', RASNG_CUTIN_CO,
         0))             AS MUSICL_RASNG_CUTIN_CO
, SUM(IF(GENRE_CD = 'CCCA', RASNG_CUTIN_CO,
