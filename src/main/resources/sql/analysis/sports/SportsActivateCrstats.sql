...
(BASE_YM, BASE_YEAR, BASE_MT, CTPRVN_CD, CTPRVN_NM, SPORTS_MATCH_CO, SPORTS_VIEWNG_NMPR_CO, POPLTN_PER_VIEWNG_NMPR_CO, SPORTS_VIEWNG_NMPR_CO_SCORE, GNRLZ_SCORE, STDR_SPORTS_MATCH_CO, STDR_SPORTS_VIEWNG_NMPR_CO)
SELECT
    DATA.BASE_DE AS BASE_YM
    , DATA.BASE_YEAR
    , DATA.BASE_MONTH
    , DATA.CTPRVN_CD
    , DATA.CTPRVN_NM
    ...
    , (data.POPLTN_PER_VIEWNG_NMPR_CO) / STD.STD_VALUE * 100 as SPORTS_VIEWNG_NMPR_CO_SCORE
    , (data.POPLTN_PER_VIEWNG_NMPR_CO) / STD.STD_VALUE * 100 as GNRLZ_SCORE
    , STD.MATCH_CO
    , STD.VIEWNG_NMPR_CO
FROM (
    SELECT
        CONCAT(BASE_YEAR, BASE_MT) AS BASE_DE
...
JOIN (
    SELECT
        ...
        , ((SUM(SPORTS_VIEWNG_NMPR_CO) / 12 * 1000) / (select C.POPLTN_CO from ctprvn_accto_popltn_info as C where C.CTPRVN_CD = '11' and C.BASE_YEAR = '2022' and C.BASE_MT = '12')) as STD_VALUE
    FROM colct_sports_viewng_info AS S
