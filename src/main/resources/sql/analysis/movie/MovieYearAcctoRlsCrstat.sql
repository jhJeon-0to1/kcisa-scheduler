INSERT INTO movie_year_accto_rls_crstat
...
SELECT (DATA.BASE_YEAR)
     ...
     , (DATA.METRP_AREA_AT)
FROM ((SELECT T.BASE_YEAR                       AS BASE_YEAR
      ...
      (SELECT BASE_YEAR                           AS BASE_YEAR
       ...
       where M.BASE_YEAR = ?)) AS DATA
         JOIN (SELECT BASE_YEAR
               ...
               FROM (SELECT SUBSTR(RLS_YEAR, 1, 4) AS BASE_YEAR
                          , REPRSNT_GENRE_NM
                          , COUNT(*)               AS MOVIE_CO
