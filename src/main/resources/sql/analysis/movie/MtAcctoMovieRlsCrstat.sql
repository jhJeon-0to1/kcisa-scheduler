INSERT INTO movie_mt_accto_rls_crstat
...
FROM ((SELECT T.BASE_YM                               AS BASE_YM
       ...
       FROM colct_movie_mt_accto_ctprvn_accto_stats AS T
                JOIN colct_movie_mt_accto_sales_stats AS M
                     ON T.BASE_YM = M.BASE_YM
       WHERE M.BASE_YM = ?)
      UNION ALL
      (SELECT BASE_YM                             AS BASE_YM
       ...
       where M.BASE_YM = ?)) AS DATA
         JOIN (SELECT BASE_YM
               ...
               FROM (SELECT SUBSTR(RLS_YEAR, 1, 6) AS BASE_YM
                          , REPRSNT_GENRE_NM
                          , COUNT(*)               AS MOVIE_CO
                     FROM colct_movie_info
                     WHERE REPRSNT_GENRE_NM is not null
                       and REPRSNT_GENRE_NM != ''
                       and RLS_YEAR between ? and ?
                     GROUP BY SUBSTR(RLS_YEAR, 1, 6),
                              REPRSNT_GENRE_NM) AS MOVIE
               GROUP BY BASE_YM) AS GENRE
              ON DATA.BASE_YM = GENRE.BASE_YM;